...
/* ─── Top Navigation Bar ───────────────────────────────────────────── */
function TopNav({ user, logout, unreadCount, isGuest }) {
  ...
  const navItems = isGuest ? fullNavItems.slice(0, 1) : fullNavItems

  return (
    <header className="h-16 bg-[#1E1E35] border-b border-[#2D2D4E] fixed top-0 left-0 right-0 z-50 px-4 flex items-center justify-between flex-wrap gap-2 shadow-lg shadow-black/30">
      <div className="flex items-center gap-8">
        ...
        {/* Mobile Controls (Menu Toggle & Admin) */}
        <div className="md:hidden flex items-center gap-3">
          <button 
            className="text-[var(--text-hint)] hover:text-white transition-colors"
          ...
          {!isGuest && (user?.role === 'admin' || user?.role === 'administrator') && (
            <div className="relative">
              <button
                onClick={() => setIsAdminOpen(!isAdminOpen)}
                className="flex items-center gap-0.5 text-[10px] font-bold bg-[#00D4AA]/10 text-[#00D4AA] px-1.5 py-1 rounded-md border border-[#00D4AA]/30 hover:bg-[#00D4AA]/20 transition-all"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                <svg className={`w-2.5 h-2.5 transition-transform ${isAdminOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
              </button>
              {isAdminOpen && (
                <div className="absolute left-0 top-full mt-2 w-48 bg-[#1E1E35] border border-[#2D2D4E] rounded-xl shadow-xl overflow-hidden py-1 z-50">
      ...
      <div className="flex items-center gap-3">
        ...
        {!isGuest && (user?.role === 'admin' || user?.role === 'administrator') && (
          <div className="relative hidden md:flex">
            ...
            >
              Admin Dashboard {icons.ChevronDown}
            </button>
            {isAdminOpen && (
        ...
        {!isGuest && <div className="w-px h-6 bg-[#2D2D4E] mx-1"></div>}
        ...
        <div className="relative ml-1">
          ...
          <Link
            to="/login"
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-xl bg-gradient-to-r from-[#6C63FF] to-[#00D4AA] text-white text-sm font-bold shadow-[0_0_15px_rgba(108,99,255,0.4)] hover:shadow-[0_0_20px_rgba(108,99,255,0.6)] hover:opacity-90 transition-all duration-300"
          >
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-[#1E1E35] border-b border-[#2D2D4E] shadow-xl md:hidden flex flex-col p-4 gap-2 z-40">
          {navItems.map(item => {
            const isActive = location.pathname === item.to || location.pathname.startsWith(item.to + '/')
            return (
              <Link
          ...
          {!isGuest && (
            <Link 
              to="/projects/create" 
              onClick={() => setIsMobileMenuOpen(false)}
              className="mt-2 text-center btn-primary bg-[#6C63FF] hover:bg-[#4F46E5] text-white py-2 px-4 shadow-[0_0_15px_rgba(108,99,255,0.4)] rounded-xl"
            >
              {t('nav.createProject')}
            </Link>
          )}
...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    ...
    if (user && !isGuest) fetchCounts()
  }, [user, isGuest])

  return (
    <div className="min-h-screen font-sans flex flex-col" style={{ backgroundColor: 'var(--bg-page)', color: 'var(--text-primary)' }}>
      <TopNav user={user} logout={logout} unreadCount={unreadCount} isGuest={isGuest} />

      {/* pt-16 offsets the fixed navbar height — overflow on body so sticky works */}
      <div className="flex-1 pt-16">
        <div className={`max-w-[1400px] mx-auto w-full grid grid-cols-1 ${!isGuest && isHome ? 'lg:grid-cols-12' : !isGuest ? 'lg:grid-cols-12' : ''} gap-4 xl:gap-6 px-4 py-6 items-start`}>
          {!isGuest && (
             <div className="hidden lg:block lg:col-span-3">
               <LeftSidebar user={user} projectCount={projectCount} connectionCount={connectionCount} skillsData={skillsData} />
             </div>
          )}
          <main className={`min-h-screen pb-20 w-full min-w-0 ${!isGuest && isHome ? 'lg:col-span-6' : !isGuest ? 'lg:col-span-9' : 'lg:col-span-12'}`}>
            {children}
          </main>
          {!isGuest && isHome && (
             <div className="hidden lg:block lg:col-span-3">
