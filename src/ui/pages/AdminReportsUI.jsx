      ...
      {items.length === 0 ? (
        <div className="card text-center py-16 border-2 border-dashed border-[var(--border-color)]">
          <div className="w-16 h-16 bg-[var(--bg-hover)] rounded-full flex items-center justify-center mx-auto mb-4 border border-[var(--border-color)]">
            <svg className="w-8 h-8 text-[var(--text-hint)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
          </div>
          <h3 className="font-bold text-[var(--text-primary)] text-lg mb-1">No reports found</h3>
          <p className="text-[var(--text-secondary)] text-sm">No reports match the selected filter.</p>
        </div>
      ) : (
            ...
            return (
              <div key={it.id} className="card p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:border-brand-primary/30 transition-colors group">
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-[var(--text-primary)] text-base group-hover:text-brand-primary transition-colors truncate">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm text-[var(--text-secondary)]">
                    <span className="flex items-center gap-1">
                      ...
                      Reporter: <span className="font-medium text-[var(--text-primary)]">{it.reporter?.full_name || it.reporter?.name || it.reporter_id || 'Unknown'}</span>
                    </span>
                    <span className="flex items-center gap-1">
                      ...
                      About: <span className="font-medium text-[var(--text-primary)]">{it.reported_user?.full_name || it.reported_user?.name || it.target_type || 'Unknown'}</span>
                    </span>
                  </div>
                </div>
