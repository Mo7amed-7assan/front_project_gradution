      ...
      <div className="card p-6 border-brand-primaryLight shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-100">
           <div className="flex items-center gap-2">
             ...
             <div>
               <h2 className="text-lg font-bold text-slate-900">Configuration Registry</h2>
               <p className="text-xs text-slate-500 font-medium">{items.length} active variables</p>
             </div>
           </div>
           ...
           <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="form-input pl-9 py-2 text-sm w-full bg-slate-50 focus:bg-white transition-colors"
              />
           </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 px-4 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50">
            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-slate-100">
               <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" /></svg>
            </div>
            <h3 className="text-base font-bold text-slate-800 mb-1">No settings found</h3>
            <p className="text-sm text-slate-500">Could not find any configuration matching your search.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {items.map((it) => (
              <Link key={it.key} to={`/admin/settings/${it.key}`} className="group card p-5 border-slate-100 hover:border-brand-primary/30 hover:shadow-lg hover:shadow-brand-primary/5 transition-all flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex items-center gap-2">
                       ...
                       <div className="font-bold text-slate-900 font-mono text-sm group-hover:text-brand-primary transition-colors truncate" title={it.key}>{it.key}</div>
                    </div>
                  ...
                  <p className="text-sm text-slate-500 mb-4 line-clamp-2">{it.description || 'No description provided for this setting.'}</p>
                </div>
                
                <div className="pt-4 border-t border-slate-100 bg-slate-50 -mx-5 -mb-5 px-5 py-3 rounded-b-2xl flex items-center justify-between">
                  <div className="flex-1 min-w-0 pr-4">
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Current Value</div>
                    <div className="font-mono text-xs text-slate-700 truncate bg-white px-2 py-1.5 rounded-lg border border-slate-200">
                       {String(it.value)}
                    </div>
                  </div>
