        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-[var(--border-color)]">
           <div className="flex items-center gap-2">
             ...
             <div>
               ...
               <p className="text-xs text-[var(--text-secondary)] font-medium">{items.length} total users</p>
             </div>
           </div>
           
           <div className="flex items-center gap-3 w-full sm:w-auto">
             <div className="relative w-full sm:w-48">
                <svg className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-hint)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
                <select 
                  value={filterRole} 
                  onChange={(e) => setFilterRole(e.target.value)} 
                  className="form-select pl-9 py-2 text-sm w-full bg-[var(--bg-hover)] focus:bg-white transition-colors"
                >
        ...
        {loading ? (
          ...
          <div className="text-center py-12 px-4 border-2 border-dashed border-[var(--border-color)] rounded-3xl bg-[var(--bg-hover)]">
            <div className="w-16 h-16 bg-[var(--bg-surface)] rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-[var(--border-color)]">
               <svg className="w-8 h-8 text-[var(--text-hint)]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            </div>
            <h3 className="text-base font-bold text-[var(--text-primary)] mb-1">No users found</h3>
            <p className="text-sm text-[var(--text-secondary)]">Try adjusting your role filter or checking back later.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((it) => {
              ...
              return (
                <Link key={it.id} to={`/admin/users/${it.id}`} className="group relative card p-5 border-[var(--border-color)] hover:border-brand-primary/30 hover:shadow-lg hover:shadow-brand-primary/5 transition-all flex flex-col justify-between">
                  <div>
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 border border-white shadow-sm flex items-center justify-center text-[var(--text-secondary)] font-bold shrink-0 overflow-hidden relative group-hover:scale-105 transition-transform">
                    ...
                    <div className="mb-2 pr-4">
                      <h3 className="font-bold text-[var(--text-primary)] text-base truncate group-hover:text-brand-primary transition-colors">{it.full_name || it.name || 'Unknown User'}</h3>
                      <p className="text-sm text-[var(--text-secondary)] truncate">{it.email}</p>
                    </div>
                  </div>
                  
                  <div className="pt-4 mt-2 border-t border-[var(--border-color)] flex items-center justify-between text-xs text-[var(--text-hint)] font-medium">
