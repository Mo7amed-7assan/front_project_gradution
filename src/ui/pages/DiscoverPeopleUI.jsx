    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="card p-2 shadow-sm border-brand-primaryLight flex gap-2 w-fit">
        <button
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${activeTab === 'search' ? 'bg-brand-primary text-white shadow-md shadow-brand-primary/20' : 'text-[var(--text-secondary)] hover:bg-slate-100 hover:text-slate-900'}`}
        >
          Search Community
        </button>
        <button
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2 ${activeTab === 'suggestions' ? 'bg-brand-primary text-white shadow-md shadow-brand-primary/20' : 'text-[var(--text-secondary)] hover:bg-slate-100 hover:text-slate-900'}`}
        >
      ...
      <div className="space-y-6">
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <svg className="h-5 w-5 text-[var(--text-hint)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </div>
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search by name, username, or skills..."
                className="form-input pl-11 py-3 bg-[var(--bg-hover)] w-full text-base focus:bg-white transition-colors"
              />
            </div>
            <div className="flex gap-2">
              ...
              {query && (
                <button type="button" onClick={handleClear} className="btn-secondary px-6 shrink-0 text-[var(--text-secondary)] hover:text-slate-900">
                  Clear
                </button>
              )}
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 px-4 border-2 border-dashed border-[var(--border-color)] rounded-3xl bg-[var(--bg-hover)]">
            <div className="w-20 h-20 bg-[var(--bg-surface)] shadow-sm rounded-full flex items-center justify-center mx-auto mb-4 border border-[var(--border-color)]">
              <svg className="w-10 h-10 text-[var(--text-hint)]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
              </svg>
            </div>
            <h3 className="text-xl font-bold text-[var(--text-primary)] mb-1">No users found</h3>
            <p className="text-sm text-[var(--text-secondary)]">Try adjusting your search terms or filters.</p>
          </div>
        ) : users.length > 0 ? (
          <>
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <h3 className="text-lg font-bold text-[var(--text-primary)] group-hover:text-brand-primary transition-colors truncate">
                        {user.full_name || user.username}
                      </h3>
                      <p className="text-sm font-medium text-[var(--text-secondary)] truncate mb-4">@{user.username}</p>
                    </div>

                    <div className="space-y-2 pt-4 border-t border-[var(--border-color)]">
                      <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
                        <svg className="w-4 h-4 text-[var(--text-hint)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
