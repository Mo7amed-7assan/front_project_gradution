...
const statusConfig = {
  ...
  withdrawn: { bg: 'bg-slate-100 text-slate-600 border-slate-200',       dot: 'bg-slate-400' },
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-slate-600 mt-1 flex items-center gap-1.5">
            ...
            {projectId ? (
              ...
              <span className="font-semibold text-slate-700">{getProjectTitle(invitation)}</span>
            )}
          </p>

          {invitation.message && (
            <div className="mt-2 p-3 bg-slate-50 rounded-xl border border-slate-100 text-sm text-slate-600 italic">
              "{invitation.message}"
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            ...
            {(invitation.invitation_type || invitation.role) && (
              <span className="text-xs font-bold text-slate-500 px-2.5 py-1 bg-slate-100 rounded-full capitalize">
        ...
        <div className="flex items-center gap-2 shrink-0 self-start sm:self-center">
          {mode === 'received' && isPending(invitation.status) && (
            <>
              ...
              <button
                ...
                disabled={isProcessing}
                className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 hover:border-rose-200 rounded-xl text-xs font-bold transition-colors disabled:opacity-50"
              >
          ...
          {mode === 'sent' && isPending(invitation.status) && (
            <button
              ...
              disabled={isProcessing}
              className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition-colors disabled:opacity-50"
            >
          ...
          <div key={i} className="card p-5 animate-pulse">
            <div className="flex gap-4">
              ...
              <div className="flex-1 space-y-2">
                <div className="h-5 bg-slate-100 rounded-xl w-2/5" />
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-sm font-bold transition-all ${
              activeTab === tab.id
                ? 'bg-white text-brand-primary shadow-sm shadow-slate-200'
          ...
          >
            ...
            <span className={`ml-1 px-2 py-0.5 rounded-full text-xs font-black ${
              activeTab === tab.id ? 'bg-brand-primary text-white' : 'bg-slate-200 text-slate-600'
            }`}>
              {tab.count}
            </span>
          </button>
      ...
      {currentList.length === 0 ? (
        <div className="card text-center py-20 px-4 border-2 border-dashed border-slate-200">
          <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6 border border-slate-100 shadow-sm">
            {activeTab === 'received'
              ...
              : <svg className="w-9 h-9 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>
            }
          </div>
