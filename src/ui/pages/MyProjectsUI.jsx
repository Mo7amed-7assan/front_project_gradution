    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-2">
        {loading
          ...
          : projects.length === 0
          ? (
            <div className="col-span-1 sm:col-span-2 lg:col-span-3 card p-12 text-center border-dashed border-2 border-[var(--border-color)]">
              ...
              <p className="text-[var(--text-secondary)] mb-6 max-w-sm mx-auto">Create your first project and start building your team.</p>
              <Link
                to="/projects/create"
          ...
          : projects.map(p => {
            ...
            return (
              <div key={p.id} className="card p-0 flex flex-col group hover:-translate-y-1 transition-all hover:shadow-xl hover:shadow-brand-primary/10 border-[var(--border-color)] hover:border-brand-primary/30 overflow-hidden">
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <Link
                        to={`/projects/${p.id}`}
                        className="font-bold text-[var(--text-primary)] text-lg hover:text-brand-primary truncate block transition-colors leading-tight"
                      >
                        {p.title || p.name}
                      </Link>
                      {p.status && (
                  ...
                  {roles.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-2 mt-auto">
                      {roles.slice(0, 3).map(r => (
                        <span
                          key={r.id}
                          className="text-[10px] uppercase tracking-wider font-bold bg-[var(--bg-hover)] text-[var(--text-secondary)] px-2 py-1 rounded-md"
                        >
                          {r.role_name || r.name || r.title}
                        </span>
                      ))}
                      {roles.length > 3 && (
                        <span className="text-[10px] uppercase tracking-wider font-bold bg-[var(--bg-hover)] text-[var(--text-hint)] px-2 py-1 rounded-md">
                          +{roles.length - 3}
                        </span>
                      )}
                    </div>
                ...
                {/* Actions */}
                <div className="flex items-center gap-px bg-[var(--bg-hover)] mt-auto">
                  <Link
                    to={`/projects/${p.id}`}
                    className="flex-1 text-center text-sm font-bold px-2 py-3 bg-[var(--bg-surface)] text-brand-primary hover:bg-slate-50 transition-colors"
                  >
                    Open
                  </Link>
                  {relation.isOwner && (
                    <Link
                      ...
                      className="flex-1 text-center text-sm font-bold px-2 py-3 bg-[var(--bg-surface)] text-[var(--text-secondary)] hover:bg-slate-50 hover:text-slate-900 transition-colors"
                    >
                      Edit
                    </Link>
                  )}
                  {relation.isOwner && (
                    <button
                      ...
                      className="flex-1 text-sm font-bold px-2 py-3 bg-[var(--bg-surface)] text-rose-600 hover:bg-rose-50 transition-colors disabled:opacity-50"
                    >
