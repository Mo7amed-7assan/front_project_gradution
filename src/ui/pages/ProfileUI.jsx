...
export default function ProfileUI({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-brand-primaryDark h-48 sm:h-64 flex items-end p-4 md:p-8 shadow-xl shadow-brand-primary/10">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-end gap-6 w-full">
           <div className="flex flex-col items-center gap-2 shrink-0">
             <div className="relative w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-white shadow-lg overflow-hidden bg-[var(--bg-surface)]">
               <label htmlFor="profile-picture-input" className="absolute inset-0 cursor-pointer">
                 <img
                   src={profilePicturePreview || profile?.profile_picture_url || profile?.profile_picture || user?.profile_picture_url || user?.profile_picture || user?.avatar || `https://ui-avatars.com/api/?name=${profile.full_name}&background=EEEDFF&color=4F46E5`}
                 ...
                 <div className="absolute inset-0 bg-slate-900/40 opacity-0 hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-center px-2">
                   <span className="text-xs text-white uppercase tracking-[0.22em] font-semibold">Change photo</span>
                 </div>
               </label>
               <input
             ...
             {profilePicturePreview && (
               <button
                 ...
                 disabled={profilePictureUploading || profileSaving}
                 className="px-3 py-1 rounded-lg text-xs font-semibold bg-white text-brand-primary shadow hover:bg-brand-primaryLight hover:text-white transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
               >
                 {profilePictureUploading || profileSaving ? 'Saving…' : 'Save photo'}
               </button>
             )}
             {profilePictureError && (
               <div className="text-xs text-rose-300 text-center max-w-[8rem]">{profilePictureError}</div>
             )}
           </div>
      ...
      {/* Modern Tabs */}
      <div className="flex overflow-x-auto no-scrollbar gap-2 p-1 bg-[var(--bg-surface)] rounded-2xl shadow-sm border border-[var(--border-color)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              <div className="card p-4 md:p-8">
                ...
                <p className="text-[var(--text-secondary)] leading-relaxed whitespace-pre-line text-sm md:text-base">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={idx} className="flex flex-col gap-1">
                      <span className="text-xs font-bold text-[var(--text-hint)] uppercase tracking-wider">{item.label}</span>
                      {item.url ? (
                        ...
                        <span className="text-sm text-[var(--text-hint)] italic">Not specified</span>
                      )}
                    </div>
                  ))}
                  <div className="pt-2 border-t border-[var(--border-color)]">
                    <span className="text-xs font-bold text-[var(--text-hint)] uppercase tracking-wider">Email Contact</span>
                    <p className="text-sm font-medium text-[var(--text-primary)] mt-1">{profile.email || user?.email || 'Hidden'}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="card p-4 md:p-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
              <div>
                ...
                <p className="text-sm text-[var(--text-secondary)] mt-1">Showcase your expertise and level of proficiency.</p>
              </div>
            ...
            {skillsLoading ? (
              ...
              <div className="text-center py-12 px-4 border-2 border-dashed border-[var(--border-color)] rounded-3xl">
                ...
                <h3 className="text-lg font-bold text-[var(--text-primary)] mb-1">No skills documented</h3>
                <p className="text-sm text-[var(--text-secondary)] mb-4 max-w-sm mx-auto">Founders and teammates search for specific skills. Add yours to stand out.</p>
                <button onClick={() => handleSkillModalOpen()} className="btn-secondary text-brand-primary">Add Your First Skill</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {skills.map((skill) => (
                  <div key={skill.id} className="group p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-hover)] hover:bg-white hover:border-brand-primary/30 hover:shadow-lg hover:shadow-brand-primary/5 transition-all relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                       <button onClick={() => handleSkillModalOpen(skill)} className="p-1.5 rounded-lg bg-[var(--bg-hover)] text-brand-primary hover:bg-brand-primaryLight transition-colors" title="Edit">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                       </button>
                       <button onClick={() => handleDeleteSkill(skill.id)} className="p-1.5 rounded-lg bg-[var(--bg-hover)] text-rose-600 hover:bg-rose-100 transition-colors" title="Delete">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                       </button>
                    </div>
                    <h3 className="font-bold text-[var(--text-primary)] text-lg mb-2 pr-12">{skill.skill_name}</h3>
                    <div className="flex items-center gap-4 text-xs font-semibold text-[var(--text-secondary)] mb-3">
                      <span className="flex items-center gap-1">
                        <svg className="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                        Lvl {skill.proficiency_level}
                      </span>
                      <span className="flex items-center gap-1">
                        <svg className="w-4 h-4 text-[var(--text-hint)]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          ...
          <div className="card p-4 md:p-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
              <div>
                ...
                <p className="text-sm text-[var(--text-secondary)] mt-1">Showcase projects, code, designs, or articles you've created.</p>
              </div>
            ...
            {portfolioLoading ? (
               ...
               <div className="text-center py-12 px-4 border-2 border-dashed border-[var(--border-color)] rounded-3xl">
                ...
                <h3 className="text-lg font-bold text-[var(--text-primary)] mb-1">No portfolio items</h3>
                <p className="text-sm text-[var(--text-secondary)] mb-4 max-w-sm mx-auto">The best way to prove your skills is to show what you've built.</p>
                <button onClick={() => handlePortfolioModalOpen()} className="btn-secondary text-brand-primary">Create First Item</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {portfolio.map((item) => (
                  <div key={item.id} className="group flex flex-col justify-between p-6 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-surface)] shadow-sm hover:shadow-xl hover:shadow-brand-primary/10 hover:border-brand-primary/30 transition-all relative">
                    ...
                    <div>
                      ...
                      <h3 className="text-lg font-bold text-[var(--text-primary)] mb-2 leading-tight pr-12">{item.title}</h3>
                    ...
                    <div className="mt-4 pt-4 border-t border-[var(--border-color)] flex items-center justify-between flex-wrap gap-2">
                       <div className="flex items-center gap-1.5 overflow-hidden pr-2">
                         ...
                         {(item.skills?.length || 0) > 3 && <span className="text-xs text-[var(--text-hint)]">+{item.skills.length - 3}</span>}
                       </div>
            ...
            <div className="card p-4 md:p-8">
              ...
              <form onSubmit={handleSaveProfile} className="space-y-4">
                ...
                <div className="pt-4 border-t border-[var(--border-color)] space-y-4">
                  <h3 className="text-sm font-bold text-[var(--text-primary)]">Social Links</h3>
                  <div>
