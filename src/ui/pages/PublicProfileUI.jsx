...
export default function PublicProfileUI({
  ...
  if (!profile) return (
    <div className="max-w-5xl mx-auto p-12 text-center card border-dashed border-2 border-[var(--border-color)]">
      <h3 className="text-xl font-bold text-[var(--text-primary)] mb-2">Profile not found</h3>
      <p className="text-[var(--text-secondary)]">The user you are looking for does not exist or has been removed.</p>
    </div>
  )
    ...
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="px-6 pb-6 pt-16 md:pt-28 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="flex flex-col md:flex-row md:items-end gap-6">
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-3xl bg-[var(--bg-surface)] p-1.5 shadow-xl shrink-0">
              ...
              <div className="pb-2">
                <h1 className="text-3xl font-black text-[var(--text-primary)] leading-tight">
                  {profile.full_name || profile.username}
                </h1>
                <p className="text-brand-primary font-bold text-lg">
                  @{profile.username}
                </p>
                <div className="flex items-center gap-1.5 text-[var(--text-secondary)] font-medium mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="card p-6">
            ...
            <p className="text-[var(--text-secondary)] text-sm leading-relaxed mb-6 whitespace-pre-wrap">
            ...
            <div className="space-y-4 pt-4 border-t border-[var(--border-color)]">
              {profile.email && (
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[var(--bg-hover)] flex items-center justify-center text-[var(--text-hint)] shrink-0">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-[var(--text-hint)] uppercase tracking-wider">Email</p>
                    <p className="text-sm font-medium text-[var(--text-primary)] truncate">{profile.email}</p>
                  </div>
                </div>
              )}
              {profile.website_url && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-[var(--text-hint)] uppercase tracking-wider">Website</p>
              ...
              {profile.github_url && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-[var(--text-hint)] uppercase tracking-wider">GitHub</p>
              ...
              {profile.linkedin_url && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-[var(--text-hint)] uppercase tracking-wider">LinkedIn</p>
          ...
          <div className="card p-6 border-b-4 border-amber-400">
            ...
            <div className="flex items-end gap-3 mb-2">
              <span className="text-4xl font-black text-[var(--text-primary)] leading-none">{Number(ratingsData.average).toFixed(1)}</span>
              <div className="flex flex-col pb-1">
                <div className="flex text-amber-400 text-sm">
                  {'★'.repeat(Math.round(ratingsData.average))}{'☆'.repeat(5 - Math.round(ratingsData.average))}
                </div>
                <span className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mt-0.5">{ratingsData.count} reviews</span>
              </div>
            </div>
          </div>
        </div>
            ...
            {profile.skills?.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {profile.skills.map((skill) => (
                  <div key={skill.id} className="group flex items-center bg-[var(--bg-hover)] border border-[var(--border-color)] rounded-xl overflow-hidden hover:border-brand-primary/30 transition-colors">
                    <div className="px-3 py-2 text-sm font-bold text-[var(--text-primary)]">
                      {skill.skill_name}
                    </div>
                    <div className="px-3 py-2 bg-slate-100/50 border-l border-[var(--border-color)] flex items-center gap-1.5 text-xs font-bold text-[var(--text-secondary)]">
                    ...
                    {!isOwnProfile && (
                      <button
                        ...
                        className="px-3 py-2 bg-brand-primaryLight/10 text-brand-primary hover:bg-brand-primary text-xs font-bold uppercase tracking-wider hover:text-white transition-colors border-l border-[var(--border-color)] opacity-0 group-hover:opacity-100"
                      >
                        Endorse
                      </button>
                    )}
              ...
              <p className="text-[var(--text-secondary)] text-sm p-4 bg-[var(--bg-hover)] rounded-xl border border-dashed border-[var(--border-color)] text-center font-medium">No skills listed yet.</p>
            )}
          </div>
          ...
          <div className="card p-6">
            ...
            {portfolio.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {portfolio.map((item) => (
                  <div key={item.id} className="p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-hover)] hover:bg-white hover:border-brand-primary/30 hover:shadow-lg hover:shadow-brand-primary/5 transition-all group flex flex-col h-full">
                    <div className="flex items-start justify-between gap-4 mb-2">
                      <h3 className="font-bold text-[var(--text-primary)] group-hover:text-brand-primary transition-colors truncate">
                        {item.title}
                      </h3>
                      {item.external_url && (
                        <a href={item.external_url} target="_blank" rel="noreferrer" className="w-8 h-8 rounded-full bg-[var(--bg-surface)] flex items-center justify-center text-[var(--text-hint)] hover:text-brand-primary shadow-sm shrink-0 transition-colors">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                        </a>
                      )}
                    </div>
                    <p className="text-xs font-bold text-[var(--text-hint)] uppercase tracking-wider mb-3">{item.item_type || 'Project'}</p>
                    ...
                    {item.skills?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-auto">
                        {item.skills.slice(0, 3).map((skill) => (
                          <span key={skill} className="px-2 py-1 bg-[var(--bg-surface)] text-[var(--text-secondary)] border border-[var(--border-color)] rounded-md text-[10px] font-bold uppercase tracking-wider">
                            {skill}
                          </span>
                        ))}
                        {item.skills.length > 3 && (
                          <span className="px-2 py-1 bg-[var(--bg-hover)] text-[var(--text-secondary)] rounded-md text-[10px] font-bold uppercase tracking-wider">
                            +{item.skills.length - 3}
                          </span>
                        )}
                      </div>
              ...
              <p className="text-[var(--text-secondary)] text-sm p-4 bg-[var(--bg-hover)] rounded-xl border border-dashed border-[var(--border-color)] text-center font-medium">No portfolio items added yet.</p>
            )}
          </div>
            ...
            {ratingsData.items.length > 0 ? (
              <div className="space-y-4">
                {ratingsData.items.map(r => (
                  <div key={r.id} className="p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-hover)]">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4 pb-4 border-b border-slate-200/60">
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <span className="font-bold text-[var(--text-primary)]">{r.rater?.full_name || r.rater?.name || 'Unknown User'}</span>
                          <div className="flex items-center bg-[var(--bg-surface)] px-2 py-0.5 rounded-full shadow-sm border border-[var(--border-color)] text-xs">
                            <span className="text-amber-400 mr-1">★</span>
                            <span className="font-bold text-[var(--text-primary)]">{Number(getRatingScore(r)).toFixed(1)}</span>
                          </div>
                        </div>
                        {r.project && <p className="text-xs font-medium text-[var(--text-secondary)]">Collaborated on: <span className="text-brand-primary">{r.project.title || r.project.name}</span></p>}
                      </div>
                      ...
                      {currentUser && (r.rater?.id === currentUser.id || r.rater_id === currentUser.id) && (
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          }} className="text-xs font-bold px-3 py-1.5 rounded-lg bg-[var(--bg-surface)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-brand-primary hover:border-brand-primary/30 transition-colors">
                            Edit
                          </button>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                      {RATING_FIELDS.filter((field) => field.key !== 'overall_rating').map((field) => (
                        <div key={field.key} className="bg-[var(--bg-surface)] p-2 rounded-lg border border-[var(--border-color)] flex items-center justify-between">
                          <span className="text-[10px] font-bold text-[var(--text-hint)] uppercase tracking-wider">{field.label}</span>
                    ...
                    {getRatingFeedback(r) && (
                      <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border-color)] relative">
                        ...
                        <p className="text-sm text-[var(--text-secondary)] leading-relaxed italic pl-2 relative z-10">
                          {getRatingFeedback(r)}
                        </p>
                      </div>
                    )}
              ...
              <p className="text-[var(--text-secondary)] text-sm p-4 bg-[var(--bg-hover)] rounded-xl border border-dashed border-[var(--border-color)] text-center font-medium">No reviews yet.</p>
            )}
          </div>
        </div>
      </div>
      ...
      {showInviteModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-[var(--bg-surface)] rounded-3xl max-w-md w-full p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-[var(--text-primary)]">Invite to Project</h3>
              <button onClick={() => setShowInviteModal(false)} className="w-8 h-8 rounded-full bg-[var(--bg-hover)] flex items-center justify-center text-[var(--text-hint)] hover:text-slate-700 hover:bg-slate-100 transition-colors">
            ...
            {myProjects.length === 0 ? (
              <div className="text-center py-8">
                ...
                <h4 className="text-lg font-bold text-[var(--text-primary)] mb-2">No active projects</h4>
                <p className="text-[var(--text-secondary)] text-sm mb-6">You need to create a project before you can send invitations.</p>
      ...
      {editingRating && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-[var(--bg-surface)] rounded-3xl max-w-lg w-full p-8 shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-[var(--text-primary)]">Edit Rating</h3>
              <button onClick={() => setEditingRating(null)} className="w-8 h-8 rounded-full bg-[var(--bg-hover)] flex items-center justify-center text-[var(--text-hint)] hover:text-slate-700 hover:bg-slate-100 transition-colors">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            
            <div className="space-y-6 mb-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 p-5 bg-[var(--bg-hover)] rounded-2xl border border-[var(--border-color)]">
                {RATING_FIELDS.map((field) => (
                  <div key={field.key}>
                    <label className="flex items-center justify-between text-xs font-bold text-[var(--text-primary)] uppercase tracking-wider mb-2">
                      {field.label}
